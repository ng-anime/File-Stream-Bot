...
class Var(object):
    MULTI_CLIENT = False
    API_ID = int(getenv('API_ID', '15573352'))
    API_HASH = str(getenv('API_HASH', '7e6ccc6bf5b57b3393b7fe172282fcc1'))
    BOT_TOKEN = str(getenv('BOT_TOKEN' , ''))
    PICS = (environ.get('PICS', 'https://envs.sh/jUp.jpg')).split()
    name = str(getenv('name', 'hindidubbbooot_bot'))
    SLEEP_THRESHOLD = int(getenv('SLEEP_THRESHOLD', '60'))
    WORKERS = int(getenv('WORKERS', '4'))
    BIN_CHANNEL = int(getenv('BIN_CHANNEL', '-1002477005688'))
    NEW_USER_LOG = int(getenv('NEW_USER_LOG', '-1002477005688'))
    PORT = int(getenv('PORT', '8080'))
    BIND_ADRESS = str(getenv('WEB_SERVER_BIND_ADDRESS', '0.0.0.0'))
    PING_INTERVAL = int(environ.get("PING_INTERVAL", "1200"))  # 20 minutes
    OWNER_ID = [int(x) for x in os.environ.get("OWNER_ID", "7613604632").split()]
    NO_PORT = bool(getenv('NO_PORT', False))
    APP_NAME = None
    OWNER_USERNAME = str(getenv('OWNER_USERNAME', 'nahidhasan_990'))
    if 'DYNO' in environ:
        ON_HEROKU = True
